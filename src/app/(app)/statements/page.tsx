...
interface BankTransaction {
  ...
  payment: { id: string; folio: string } | null;
}
...
const editInput =
  "w-full border border-indigo-300 rounded px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500";
...
interface MoveStatementModalProps {
  ...
  onMergeIfPeriodExistsChange: (v: boolean) => void;
  onClose: () => void;
  onSubmit: () => void;
}

function MoveStatementModal({
  statement, accounts, targetAccountId, mergeIfPeriodExists, loading, error,
  onTargetAccountChange, onMergeIfPeriodExistsChange, onClose, onSubmit,
      ...
      {statement && (
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="space-y-1">
            ...
            >
              {accounts
                .filter((a) => a.id !== statement.accountId)
                .map((a) => (
          ...
          <div className="flex flex-col-reverse gap-2 pt-2 sm:flex-row sm:justify-end">
            ...
            <button type="button" onClick={onSubmit} className="btn-primary" disabled={loading || !targetAccountId}>
              {loading
...
function MergeAccountsModal({
  sourceAccount, accounts, targetAccountId, loading, error,
  onTargetAccountChange, onClose, onSubmit,
...
export default function HouseholdStatementsPage() {
    ...
    try {
      ...
      setSelectedStatement((current) => {
        const preferred = preferredStatementId
          ? sorted.find((s: BankStatement) => s.id === preferredStatementId)
          : null;
        if (preferred) return preferred;
        if (current) return sorted.find((s: BankStatement) => s.id === current.id) ?? sorted[0] ?? null;
  ...
  const visibleTransactions = useMemo(() => {
    if (sentFilter === "sent") return transactions.filter((t) => !!t.payment);
    if (sentFilter === "not_sent") return transactions.filter((t) => !t.payment);
    return transactions;
  }, [transactions, sentFilter]);

  const selectableTransactions = useMemo(
    () => visibleTransactions.filter((t) => !t.payment),
    [visibleTransactions]
  );
  ...
  async function handleSendToPayments() {
    ...
    try {
      const res = await fetch("/api/payments/from-transactions", {
        method: "POST",
      ...
      if (created > 0 && skippedDuplicates > 0) {
        toast.success(
          <span>
            ...
            <a href="/payments" className="underline font-semibold">Ver en Pagos del Hogar</a>
          </span>,
          { duration: 5000 }
        );
      } else if (created > 0) {
        ...
        toast(`${skippedDuplicates} movimiento${skippedDuplicates !== 1 ? "s" : ""} ya estaba${skippedDuplicates !== 1 ? "n" : ""} en Pagos del Hogar`, { icon: "ℹ️" });
      }
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Error al enviar movimientos");
    } finally {
      setSendingToPayments(false);
    }
  }

  function openMoveStatement(s: BankStatement) {
    const target = accountOptions.find((a) => a.id !== s.accountId);
    setMoveStatement(s);
  ...
  function openMergeAccount(account: BankAccount) {
    const target = accountOptions.find((a) => a.id !== account.id);
  ...
  async function handleMoveStatement() {
    ...
    setMoveError(null);
    try {
      const res = await fetch(`/api/statements/${moveStatement.id}/move`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ targetAccountId: moveTargetAccountId, mergeIfPeriodExists }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error ?? "No se pudo mover el estado de cuenta");
  ...
  async function handleMergeAccounts() {
    ...
    try {
      const res = await fetch("/api/accounts/merge", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sourceAccountId: mergeSourceAccount.id, targetAccountId: mergeTargetAccountId }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error ?? "No se pudieron fusionar las cuentas");
  ...
  async function handleDeleteStatement() {
    ...
    try {
      const res = await fetch(`/api/statements/${deletingStatement.id}`, { method: "DELETE" });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error ?? "No se pudo eliminar el estado de cuenta");
      ...
      toast.error(error instanceof Error ? error.message : "No se pudo eliminar el estado de cuenta");
    } finally {
  ...
  async function saveTxnEdit() {
    ...
    setSavingTxn(true);
    try {
      const res = await fetch(`/api/transactions/${editingTxnId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async function saveMobile() {
    ...
    try {
      ...
      if (mobileTxn === "new") {
        ...
        res = await fetch(`/api/transactions/${mobileTxn.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-6">
      <Header
        ...
        subtitle="Movimientos bancarios compartidos del hogar"
        actions={
          <>
            <button
              type="button"
      ...
      {showImport && (
        <StatementImportCard
          importEndpoint="/api/statements/import"
      ...
      {loadingStatements ? (
        ...
        <div className="card text-center py-16">
          <svg className="w-12 h-12 text-gray-300 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <polyline points="17 21 17 13 7 13 7 21" strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} />
            <polyline points="7 3 7 8 15 8" strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} />
          </svg>
          <p className="text-gray-700 font-semibold text-base">Importa el primer estado de cuenta del hogar</p>
          <p className="text-gray-400 text-sm mt-1 max-w-sm mx-auto">
            Centraliza los movimientos compartidos para conciliarlos con los pagos del hogar.
          </p>
          <button
            type="button"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-1 space-y-4">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider px-1">
              Cuentas ({statementsByAccount.length})
            </p>
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            {activeSelection.length > 0 && (
              <div className="rounded-xl border border-indigo-200 bg-indigo-50 px-4 py-3 flex flex-wrap items-center gap-3">
                ...
                <button
                  ...
                  aria-label="Enviar movimientos seleccionados a Pagos del Hogar"
                  disabled={sendingToPayments}
                >
                  {sendingToPayments ? (
                    <>
            ...
            <div className="card p-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <select
                  className="input"
                  aria-label="Filtrar por estado de envío a Pagos del Hogar"
                  value={sentFilter}
            ...
            <div className="card overflow-hidden">
              ...
              ) : visibleTransactions.length === 0 && !isAddingRow ? (
                ...
                <>
                  ...
                  <div className="sm:hidden divide-y divide-gray-100">
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="min-w-0 flex-1">
                              <div className="flex items-start gap-2 flex-wrap">
                                ...
                                {alreadySent ? (
                                  <a
                                    href={`/payments?search=${t.payment!.folio}`}
                    ...
                    <div className="px-4 py-3 bg-gray-50 border-t flex justify-between items-center text-sm font-semibold">
                      <span className="text-gray-600">{visibleTransactions.length} movimientos</span>
                    ...
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50">
                        <tr className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">
                          ...
                          <th className="px-4 py-3 text-right w-28">Cargo</th>
                      ...
                      <tfoot className="bg-gray-50 border-t border-gray-200">
                        <tr>
                          <td colSpan={4} className="px-4 py-3 text-sm font-semibold text-gray-600">
                            {visibleTransactions.length} movimiento{visibleTransactions.length !== 1 ? "s" : ""}
                          </td>
      ...
      <MoveStatementModal
        ...
        onClose={() => { if (!movingStatement) setMoveStatement(null); }}
      ...
      <MergeAccountsModal
        ...
        onClose={() => { if (!mergingAccounts) setMergeSourceAccount(null); }}
        onSubmit={handleMergeAccounts}
      />

      <ConfirmDialog
        open={!!deletingStatement}
        onClose={() => { if (!deletingStatementLoading) setDeletingStatement(null); }}
      ...
      {/* Modal editar/agregar movimiento */}
      <Modal
        open={mobileTxn !== null}
      ...
      >
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="space-y-1">
            ...
            <input id="edit-productName" className="input w-full" placeholder="Ej. Cuenta Corriente"

 ...
 * Moves a bank statement to another owned account, optionally merging when the
 * destination already has the same period.
 */
export async function PATCH(
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!sourceStatement) {
        throw new RouteError(404, "No encontrado");
      }
      ...
      if (sourceStatement.accountId === targetAccountId) {
        return {
          movedTransactions: 0,
          skippedDuplicates: 0,
          targetStatementId: sourceStatement.id,
        };
      }
      ...
      if (!mergeIfPeriodExists) {
        throw new RouteError(409, PERIOD_CONFLICT_MESSAGE);
      }
    ...
    console.error("[personal/statements move PATCH]", error);

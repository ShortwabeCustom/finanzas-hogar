 ...
 * Merges two HOUSEHOLD BankAccounts: moves all statements from source to
 * target, deduplicating by txnHash. Deletes source if it becomes empty.
 */
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      const [sourceAccount, targetAccount] = await Promise.all([
        tx.bankAccount.findFirst({ where: { id: sourceAccountId, scope: "HOUSEHOLD" }, select: { id: true } }),
        tx.bankAccount.findFirst({ where: { id: targetAccountId, scope: "HOUSEHOLD" }, select: { id: true } }),
      ]);

      if (!sourceAccount) throw new RouteError(404, "Cuenta origen no encontrada");
      ...
      const remaining = await tx.bankStatement.count({ where: { accountId: sourceAccountId } });
      if (remaining === 0) {
        await tx.bankAccount.delete({ where: { id: sourceAccountId } });
      }

      return { mergedStatements, movedTransactions, skippedDuplicates };
    });
    ...
    console.error("[accounts merge POST]", error);

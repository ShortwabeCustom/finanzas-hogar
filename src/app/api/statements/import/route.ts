...
function logVisionOcrFailure(err: VisionOcrError, status: number, file: Blob, fileName: string) {
  console.warn("[statements/import HOUSEHOLD] vision_ocr_failed", {
    errorName: err.name,
    status,
...
function logUnexpectedVisionFailure(err: unknown, file: Blob, fileName: string) {
  console.error("[statements/import HOUSEHOLD] vision_ocr_unhandled_error", {
 ...
 *
 * Household variant of the import pipeline. Accepts PDF or XML.
 * Creates BankAccount with scope=HOUSEHOLD — shared across all users.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      console.error("[statements/import HOUSEHOLD] pdf-parse error", err);
      return NextResponse.json(
    ...
    if (isScannedPdf) {
      ...
      try {
        ...
        return NextResponse.json(
          { error: "No se pudo procesar el PDF escaneado con OCR. Intenta de nuevo o descarga el PDF digital desde Santander." },
          { status: 502 }
        );
      }
    ...
    try {
      ...
      console.error("[statements/import HOUSEHOLD] AI parser error", err);
    ...
    if (error instanceof Prisma.PrismaClientValidationError) {
      console.error("[statements/import HOUSEHOLD] Prisma validation error", error.message);
      return NextResponse.json(
    ...
    console.error("[statements/import HOUSEHOLD]", error);
...
async function runImport(payloads: ImportStatementPayload[], userId: string): Promise<ImportSummary> {
  const results = await Promise.all(
    payloads.map(async (p) => {
      try {
        return await importStatement(p, userId, "HOUSEHOLD");
      } catch (err: unknown) {

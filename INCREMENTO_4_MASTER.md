# INCREMENTO 4: STATEMENTS UI, TESTS UNITARIOS Y NOTIFICACIONES — PROMPT MASTER

**Fase:** Tests + UI refinamiento + Notificaciones básicas  
**Duración esperada:** 2-3 sesiones (1 statements UI + tests, 1 notificaciones, 1 QA)  
**Commits esperados:**
1. `feat(statements): add link-transaction modal UI`
2. `test(debts): unit tests for debt-calculations and validation`
3. `feat(notifications): add debt due-date alerts (email/whatsapp ready)`

**Estado previo:** INCREMENTO 3 ✅ (Integración Deudas + Analytics + E2E base)

---

## 🎯 OBJETIVO

Completar la integración de "Deudas y Préstamos" con:

1. **Statements UI** — Modal para vincular transacciones bancarias a deudas (usar API existente)
2. **Tests Unitarios** — Cobertura para debt-calculations.ts, validaciones Zod, APIs
3. **Notificaciones** — Sistema base para alertas de próximo vencimiento (email ready, WhatsApp scaffolded)
4. **E2E CI** — Ejecutar suite Playwright en pipeline automático
5. **QA Completo** — Validación manual + automated
...
## 📋 SECCIÓN 1: STATEMENTS UI — VINCULAR TRANSACCIÓN A DEUDA

### 1.1 Componente: LinkTransactionModal.tsx

**Ubicación:** `src/components/personal/debts/LinkTransactionModal.tsx`
...
- ✅ Selector de deuda (filtrar PAYABLE, ACTIVE)
- ✅ Desglose opcional: capital / interés / comisiones / penalizaciones (suma debe = monto)
- ✅ Checkbox "Vincular solo parcialmente" (si suma < monto, resto es PersonalPayment independiente)
- ✅ Botón "Vincular" → POST `/api/personal/debts/[debtId]/link-transaction`
- ✅ Validación: capital ≤ saldo actual de deuda
- ✅ Toast de éxito/error
- ✅ Tracking: trackDebtEvent('debt_transaction_linked', { transaction_amount, partial: boolean })

**API Call:**
```typescript
POST /api/personal/debts/[debtId]/link-transaction
Body: {
...
### 1.2 Integración en Statements Page

**Archivo:** `src/app/(app)/personal/statements/page.tsx`

**Cambios:**
1. ✅ Agregar estado: `const [linkingTransaction, setLinkingTransaction] = useState<Transaction | null>(null)`
2. ✅ Agregar botón "Vincular a deuda" en cada fila (desktop: hover menu, mobile: siempre visible)
3. ✅ Cargar deudas: `const [debts, setDebts] = useState<DebtAccount[]>([])`
4. ✅ Renderizar LinkTransactionModal cuando linkingTransaction es no-null
5. ✅ Badge "Vinculado a deuda" si transaction.relatedDebtId (link a `/personal/debts/[id]`)
6. ✅ Filtro checkbox: "Mostrar solo transacciones no vinculadas a deudas"
...
├─ Descripción | Monto
├─ Fecha
├─ Badges: [Status] [Si linked: "Vinculado a Deuda"]
└─ Actions: [Vincular a deuda] [Más opciones]
```

**UI Pattern (desktop):**
```
Transaction Table Row
├─ Fecha | Descripción | Cargo | Crédito | Saldo | Estado | Acciones
│                                                        ├─ Editar
│                                                        ├─ Vincular a deuda
│                                                        └─ Más
└─ Si linked: Badge "Vinculado a Deuda" en columna Estado
```

---

### 1.3 Detección Automática (Sugerencia)

**Feature Optativa:** Si transacción es cargo en tarjeta de crédito (CREDIT type):
- Buscar DebtAccount con direction=PAYABLE, type=CREDIT_CARD, personalCardId que coincida
- Mostrar toast/hint: "¿Vincular esta transacción como cargo a [deuda]?" (no automático)
- CTA: Botón "Vincular sugerido"

**Implementación:**
```typescript
async function suggestDebtLinking(transaction: BankTransaction, userId: string) {
  if (!transaction.chargeAmount || transaction.chargeAmount <= 0) return null;
  
  // Find card for this account
  const card = await prisma.personalCard.findFirst({
    where: { userId, /* match account somehow */ }
  });
...
## 📊 SECCIÓN 2: TESTS UNITARIOS

### 2.1 Archivo: tests/unit/debt-calculations.test.ts
...
### 2.2 Archivo: tests/unit/debt-validation.test.ts
...
### 2.3 Archivo: tests/unit/debt-api.test.ts
...
### 3.3 Servicio: NotificationService
...
### 3.4 Background Job: Enviar Notificaciones
...
### 3.5 Endpoint Cron (Optional): Vercel Cron
...
### 3.6 UI: Settings Página Notificaciones
...
```
src/components/personal/debts/LinkTransactionModal.tsx
...
| `src/app/(app)/personal/statements/page.tsx` | Modal LinkTransactionModal, botón vincular, badge linked, filtro |
| `src/lib/analytics.ts` | Agregar evento debt_notification_sent |
| `package.json` | Scripts test:e2e, test:unit, playwright dependency |
| `.env.example` | CRON_SECRET, NOTIFICATION_SETTINGS |
| `finanzas.md` | Actualizar estado Incremento 4 |

---

## 🚀 ORDEN RECOMENDADO

1. **Día 1: Statements UI + Link Transaction Modal**
   - LinkTransactionModal.tsx
   - Integración en statements/page.tsx
   - Validación + trackDebtEvent

2. **Día 2: Unit Tests**
   - Tests unitarios (3 archivos: calculations, validation, api)
   - Coverage ≥ 80%

3. **Día 3: Notifications Base**
   - Schema DebtNotification
   - NotificationService (stubs)
   - Settings UI
...
Incremento 4 completado cuando:

1. ✅ Modal "Vincular a deuda" en Statements funcional (POST link-transaction)
2. ✅ Badge "Vinculado a deuda" muestra en transacciones
3. ✅ Filtro "Solo no vinculadas" funciona
4. ✅ Unit tests: ≥ 80% coverage (calculations, validation, API)
5. ✅ Todos los tests unitarios pasan
6. ✅ DebtNotification modelo en schema
7. ✅ NotificationService scaffolded (email/whatsapp stubs)
8. ✅ Settings UI: crear notificaciones por deuda
9. ✅ Cron endpoint ready (Vercel Cron compatible)
10. ✅ Playwright config + tests e2e (debts.spec.ts) funcionando localmente
11. ✅ GitHub Actions workflow ejecutando tests en CI
12. ✅ Test results + artifacts guardados en CI
13. ✅ Sin errores TypeScript
14. ✅ Build de producción exitoso
15. ✅ Documentación actualizada (finanzas.md)
...
- INCREMENTO 3 (previo): `/INCREMENTO_3_MASTER.md`
- Tests E2E (base): `tests/e2e/debts.spec.ts`
- Statements actual: `src/app/(app)/personal/statements/page.tsx`
- Link transaction API (Incremento 1): `/api/personal/debts/[id]/link-transaction`

---

**Fin del Prompt Master para Incremento 4**

Proceder cuando Incremento 3 sea validado y merged. Este incremento agrega UI refinement, comprehensive testing, y notificaciones base para deudas.
